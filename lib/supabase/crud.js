...
export async function getByArray(supabase, tableName, enzyme, parsedGene = []) {
  try {
    const { data, error } = await supabase.from(tableName).select("*").eq("Enzyme", enzyme).contains("Gene", parsedGene); // ใช้ contains เพราะ Gene เป็น array}
    if (error) {
      console.error("GetByArray Error: " + error.message);
      return { data: null, error: error };
    }
    return { data: data, error: null };
  } catch (err) {
    console.error("GetByArray Error: " + err.message);
    return { data: null, error: err }; //for User
  }
}
