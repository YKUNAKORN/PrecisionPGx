...
import { DecreaseFridgeItem } from '../../service/fridge_service';

/**
 * @swagger
 * /api/user/fridge/drop:
 *   patch:
 *     summary: Updated a fridge remaining item
 *     description: Decrease a fridge remaining item in the database
 *     tags:
 *       - Fridge
 *     parameters:
 *       - in: query
...
export async function PATCH(request: NextRequest) {
    ...
    if (!req.item) {
        (ResponseModel as any).status = '400';
        (ResponseModel as any).message = 'Item parameter is required';
        (ResponseModel as any).data = null;
        return NextResponse.json(ResponseModel, { status: 400 }); //for User
    }
    const { data, error } = await DecreaseFridgeItem(id, req);
    if (error) {
