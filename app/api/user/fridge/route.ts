...
export async function GET() {
    ...
    if (!data || data.length === 0) {
        ...
        (ResponseModel as any).message = 'not Found Fridges';
    ...
    if (error) {
        ...
        (ResponseModel as any).message = 'Failed to retrieve fridges' + error;
        (ResponseModel as any).data = null;
...
export async function PUT(request: NextRequest) {
    ...
    if (!id) {
        ...
        (ResponseModel as any).message = 'ID parameter is required';
        (ResponseModel as any).data = null;
    ...
    if (error) {
        (ResponseModel as any).status = '500';
        (ResponseModel as any).message = 'Failed to update fridge' + error;
        (ResponseModel as any).data = null;
        return NextResponse.json(ResponseModel, { status: 500 }) ; //for User
    }
    (ResponseModel as any).status = '200';
...
export async function DELETE(request: NextRequest) {
    ...
    if (error) {
        (ResponseModel as any).status = '500';
        (ResponseModel as any).message = 'Failed to delete fridge' + error;
        (ResponseModel as any).data = null;
        return NextResponse.json(ResponseModel, { status: 500 }) ; //for User
    }
    (ResponseModel as any).status = '200';
...
export async function POST(request: NextRequest) {
    ...
    if (error) {
        (ResponseModel as any).status = '500';
        (ResponseModel as any).message = 'Failed to create fridge' + error;
        (ResponseModel as any).data = null;
        return NextResponse.json(ResponseModel, { status: 500 }) ; //for User
    }
    (ResponseModel as any).status = '201';
    (ResponseModel as any).message = 'Creation Successful';
    (ResponseModel as any).data = data;
