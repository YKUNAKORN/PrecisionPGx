...
export async function GET(req: NextRequest): Promise<NextResponse> {request, { params }) {
...
export async function PUT(req: NextRequest): Promise<NextResponse> {request, { params }) {
    const { id } = await params
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {request, { params }) {
    const { id } = await params;
    if (!id) {
        console.error("Invalid Data: Rule ID is required") // for Debug
        ResponseModel.status = '400'
