...
export async function PUT(req: NextRequest): Promise<NextResponse> {request) {
...
export async function POST(req: NextRequest): Promise<NextResponse> {request) {
    const body = await request.json();
    const { searchParams } = new URL(request.url);
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {request) {
    const { searchParams } = new URL(request.url);
    const id = searchParams.get('id');
    if (!id) {
        ResponseModel.status = "400";
