...
import { CreateNote as CreateNoteModel } from "@/lib/model/Note"
import { CreateData, DeleteData, GetAllData, GetDataByid, UpdateData } from "@/lib/supabase/crud"
...
export async function CreateNote(row: NoteType): Promise<NoteResponse> {
    try {
        const noteModel = CreateNoteModel([
            {
                report_id: row.report_id,
                note: row.note
            }
        ])
        const { data, error } = await CreateData('note', noteModel)
        if (error) {
            throw error
        }
...
export async function GetAllNotes(): Promise<NoteResponse> {
    try {
        const { data, error } = await GetAllData('note')
        if (error) {
            throw error
        }
        return { data: data, error: null }
...
export async function GetNoteById(id: string): Promise<NoteResponse> {
    try {
        const { data, error } = await GetDataByid('note', 'id', id)
        if (error) {
            throw error
        }
        return { data: data, error: null }
...
export async function UpdateNoteByID(id: string, row: NoteType): Promise<NoteResponse> {
    try {
        const noteModel = CreateNoteModel([
            {
                report_id: row.report_id,
        ...
        const { data, error } = await UpdateData('note', 'id', id, noteModel[0])
        if (error) {
            throw error
        }
        return { data: data, error: null }
...
export async function DeleteNoteByID(id: string): Promise<NoteResponse> {
    try {
        const { data, error } = await DeleteData('note', 'id', id)
        if (error) {
            throw error
        }
        return { data: data, error: null }
