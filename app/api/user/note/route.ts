...
export async function POST(req: NextRequest) {
    ...
    if (error) {
        ...
        (ResponseModel as any).message = 'Created Failed' + error;
        (ResponseModel as any).data = null;
        return NextResponse.json(ResponseModel, { status: 500 });
    }
    (ResponseModel as any).status = '201';
    (ResponseModel as any).message = 'Created Successful';
...
export async function GET() {
    ...
    if (!data || data.length === 0) {
        ...
        (ResponseModel as any).message = 'Note Not Found with ID: ' + id;
        (ResponseModel as any).data = null;
    ...
    if (error) {
        (ResponseModel as any).status = '500';
        (ResponseModel as any).message = 'Failed to retrieve notes' + error;
        (ResponseModel as any).data = null;
...
export async function PUT(req: NextRequest) {
    ...
    const row = await req.json();
    (UpdateNote as any).method = row.method
    if (!id) {
        (ResponseModel as any).status = '400';
        (ResponseModel as any).message = 'ID is required';
        (ResponseModel as any).data = null;
    ...
    try {
        ...
        if (error) {
            (ResponseModel as any).status = '500';
            (ResponseModel as any).message = 'Update Failed' + error;
            (ResponseModel as any).data = null;
            return NextResponse.json(ResponseModel, { status: 500 });
        }
        (ResponseModel as any).status = '200';
        ...
        (ResponseModel as any).status = '500';
        (ResponseModel as any).message = 'Update Failed' + err;
        return NextResponse.json(ResponseModel, { status: 500 });
    }
}
