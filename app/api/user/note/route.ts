...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    if (!row.method) {
        ...
        return NextResponse.json(errorResponse, { status: 400 }) //for User
    }
    const { data, error } = await CreateNote(row)
    if (error) {
        const errorResponse = { ...ResponseModel }
        errorResponse.status = '500'
        errorResponse.message = 'Created Failed' + error
        errorResponse.data = null;
        return NextResponse.json(errorResponse, { status: 500 })
    }
    const successResponse = { ...ResponseModel }
    successResponse.status = '201';
    successResponse.message = 'Created Successful';
...
export async function GET(): Promise<NextResponse> {
    ...
    if (!data || data.length === 0) {
        ...
        errorResponse.message = 'Note Not Found'
        errorResponse.data = null;
        console.error("Note Not Found") //for Debug
        return NextResponse.json(errorResponse, { status: 404 }) //for User
    }
    if (error) {
        const errorResponse = { ...ResponseModel }
        errorResponse.status = '500'
        errorResponse.message = 'Failed to retrieve notes' + error
        errorResponse.data = null;
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        ...
        if (error) {
            const errorResponse = { ...ResponseModel }
            errorResponse.status = '500'
            errorResponse.message = 'Update Failed' + error
            errorResponse.data = null;
            return NextResponse.json(errorResponse, { status: 500 })
        }
        const successResponse = { ...ResponseModel }
        ...
        const errorResponse = { ...ResponseModel }
        errorResponse.status = '500'
        errorResponse.message = 'Update Failed' + err
        return NextResponse.json(errorResponse, { status: 500 })
    }
}
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
    ...
    if (error) {
        const errorResponse = { ...ResponseModel }
        errorResponse.status = '500';
        errorResponse.message = 'Delete Failed' + error;
        errorResponse.data = null;
        return NextResponse.json(errorResponse, { status: 500 });
    }
    const successResponse = { ...ResponseModel }
    successResponse.status = '200';
    successResponse.message = 'Delete Successful';
    successResponse.data = data;
    return NextResponse.json(successResponse, { status: 200 });
}
