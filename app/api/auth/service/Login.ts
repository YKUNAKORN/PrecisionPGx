...
export async function Login(email: string, password: string): Promise<NextResponse> {
  try {
    ...
    errorResponse.data = null
    console.error(process.env.NODE_ENV === 'development' ? (error as Error).message : undefined, error)
    return NextResponse.json(errorResponse, { status: 500 })
  }
}
