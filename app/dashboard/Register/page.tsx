...
import { mutateReportQueryOptions } from "../../../lib/fetch/Report";
import createBarcodeQueryOptions from "../../../lib/fetch/Barcode"; // ถ้ามี
import type { Patient as PatientBase, Report } from "../../../lib/fetch/type";
...
export default function Page() {
  ...
  // NOTE: mutateReportQueryOptions.post in your Report.ts is currently an object in some versions.
  // If Report.ts exports post as a function, use mutateReportQueryOptions.post()
  // If Report.ts.post is an object, pass it directly without calling.
  // Here we attempt to support both: if it's a function, call it; if not, pass as-is.
  ...
  const handleSaveSample = async () => {
    ...
    // Build DTO that matches your Report POST shape (the Omit you specified)
    // We use the minimal set you provided earlier:
    const now = new Date().toISOString();
    const reportDTO: Omit<
      Report,
      | "id"
      | "specimen_name"
      ...
      | "updated_at"
    > = {
      specimens_id: crypto.randomUUID(), // left blank if not applicable
      doctor_id: doctor || "",
      patient_id: activePatient.id,
      pharm_verify: false,
      medtech_verify: false,
      note_id: "", // if you have note id, set it
      rule_id: "", // if rule applied
      more_information: notes || "",
      pharmacist_id: "", // fill if known
      medical_technician_id: "", // fill if known
      request_date: now,
      report_date: now,
    };
